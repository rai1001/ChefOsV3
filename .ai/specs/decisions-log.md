...
### ADR-0022 — Compliance APPCC v0 acotado sin alérgenos, fotos ni firma digital

**Fecha**: 2026-04-28
**Estado**: aceptada
**Módulo / área afectada**: `compliance`, `procurement`, `inventory`, Next Route Handlers

#### Contexto

El caso Eurostars necesita control APPCC operativo inmediato sobre recepción, cámaras, limpieza y trazabilidad de lote. El brief histórico de compliance incluía alérgenos, fotos, firma digital, etiquetado y auditor externo, pero ese alcance no cabe en Sprint-10 sin diluir la entrega y duplicar dominios ya cubiertos por procurement/inventory.

#### Decisión

Sprint-10 implementa Compliance APPCC v0 con alcance cerrado:

- Control de calidad por recepción mediante `v3_record_goods_receipt_quality_check`.
- Registro de temperaturas por equipo mediante `v3_log_equipment_temperature`.
- Checklist de limpieza mediante `v3_complete_cleaning_check`.
- Dashboard operativo mediante `v3_get_compliance_overview`.
- Trazabilidad completa de lote mediante `v3_trace_lot`, reutilizando lotes FIFO existentes.
- Export APPCC desde Route Handler con CSV nativo y límite de 10.000 filas.

Quedan fuera de Sprint-10: alérgenos específicos de compliance, fotos, firma digital, push notifications, multi-warehouse, vista auditor externo y consumo de eventos `v3_emit_event`.

#### Razón

- El valor vendible inmediato está en poder demostrar recepción, temperatura, limpieza y trazabilidad.
- Reutilizar FIFO evita crear un segundo modelo de lotes.
- Mantener APPCC v0 sin firma/fotos reduce riesgo legal y técnico hasta validar uso real.
- El CSV cubre inspección y backoffice sin añadir dependencia. `pdf-lib` queda pospuesto porque la regla global del repo impide añadir dependencias sin aprobación explícita; si se aprueba, debe verificarse bundle minified <= 80 KB antes de activar PDF.

#### Consecuencias

- Las firmas de las 5 RPCs aplicadas en `00082_v3_compliance_appcc.sql` son contrato de producción y no se modifican desde TS.
- No se crean migraciones nuevas en esta fase; el schema real aplicado es la fuente de verdad.
- Los eventos emitidos quedan disponibles para sprint-notifications futuro, pero no se consumen ahora.
- La exportación actual usa `text/csv; charset=utf-8` y filename `chefos-appcc-{name}-{from}-{to}.csv`.

#### Verificación

- WALL-E aplicó `00082` en `dbtrgnyfmzqsrcoadcrs`, regeneró `src/types/database.ts` y dejó smoke real PASS.
- Chappie implementa capa TS, hooks, UI, export CSV fallback, tests unitarios/componentes y E2E gated `COMPLIANCE_E2E_LIVE=1`.

#### Revisable

Revisar en `compliance-advanced` si se aprueba PDF, firma digital, evidencias con foto, etiquetado o vista auditor externo. Cualquier ampliación requiere ADR o sprint explícito.
