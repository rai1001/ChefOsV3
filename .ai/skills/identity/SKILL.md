---
name: identity
description: ayuda a trabajar una tarea concreta del módulo identity de chefos v3. úsala cuando haya que definir ownership del módulo, contrato público de identidad, sesión autenticada, usuario actual, tenant actual, contexto base de acceso, restricciones mínimas de permisos, archivos a crear o modificar dentro de src/features/identity, y validación de tenancy, permisos, tests y documentación. úsala para evitar acceso caótico a identidad desde páginas, layouts o componentes y para impedir que identity absorba lógica de otros módulos.
---

# ChefOS v3 Identity

Trabaja solo sobre tareas concretas, cerradas y revisables del módulo `identity` de ChefOS v3.

Tu función es ayudar a definir, revisar o aterrizar cambios del módulo `identity` dentro de la arquitectura oficial del proyecto.

No rehagas auth global.
No mezcles identity con otros módulos.
No abras rediseños generales.
No conviertas identity en contenedor genérico de “todo lo del usuario”.
...
## Ownership del módulo `identity`

El módulo `identity` es owner de:

- sesión autenticada
- usuario actual
- tenant actual
- contexto base de acceso
- verificaciones base de permiso
- resolución mínima de contexto que otros módulos necesitan para operar

El módulo `identity` no es owner de:

- lógica funcional específica de otros módulos
- permisos operativos detallados de `inventory`, `production`, `commercial`, etc.
...
5. No metas lógica de `identity` en `src/app/`.
6. No metas lógica de `identity` en `src/components/`.
7. No uses `src/lib/` como destino por defecto para lógica de identidad.
8. No ignores `tenant current` como concern central del módulo.
9. No asumas permisos o acceso sin declararlos explícitamente.
...
### 2. Validar ownership dentro de `identity`

Debes responder:

- si la tarea pertenece realmente a `identity`
...
No permitas que `identity` absorba lógica ajena por comodidad.
...
- cómo se resuelve sesión actual
- cómo se resuelve usuario actual
- cómo se resuelve tenant actual
- qué comprobación base de acceso existe
...
- deja claro qué pertenece y qué no pertenece a `identity`
