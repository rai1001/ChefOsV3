---
name: recipes
description: ayuda a trabajar una tarea concreta del módulo recipes de chefos v3. úsala cuando haya que definir ownership del módulo, contrato público de recetas, receta base, composición mínima, estructura de ingredientes o componentes, estado base de receta, archivos a crear o modificar dentro de src/features/recipes, y validación de permisos, tenancy, tests, documentación y trazabilidad en git. úsala para evitar mezclar recipes con catalog, procurement, inventory o production y para impedir acceso caótico al dominio de recetas desde páginas o componentes.
---

# ChefOS v3 Recipes

Trabaja solo sobre tareas concretas, cerradas y revisables del módulo `recipes` de ChefOS v3.

Tu función es ayudar a definir, revisar o aterrizar cambios del módulo `recipes` dentro de la arquitectura oficial del proyecto.

No rehagas el dominio de recetas completo.
No mezcles `recipes` con otros módulos.
No abras rediseños generales.
No conviertas `recipes` en contenedor genérico de “todo lo culinario”.
...
- `.ai/sprints/sprint-03-recipes.md`
...
## Ownership del módulo `recipes`

El módulo `recipes` es owner de:

- receta base
- composición mínima de receta
- estructura de ingredientes o componentes de receta
- estado base de receta
- validaciones mínimas del dominio de recetas
...
5. No metas lógica de `recipes` en `src/app/`.
6. No metas lógica de `recipes` en `src/components/`.
7. No uses `src/lib/` como destino por defecto para lógica de recetas.
8. No mezcles reglas de `recipes` con `catalog`, `procurement`, `inventory` o `production` sin contrato público claro.
...
### 2. Validar ownership dentro de `recipes`

Debes responder:

- si la tarea pertenece realmente a `recipes`
...
No permitas que `recipes` absorba lógica ajena por comodidad.
...
- cuál es la receta base afectada
- cuál es su composición mínima
...
- deja claro qué pertenece y qué no pertenece a `recipes`
- protege la frontera pública del módulo
- evita acceso disperso al dominio de recetas desde cualquier capa
