---
name: inventory
description: ayuda a trabajar una tarea concreta del módulo inventory de chefos v3. úsala cuando haya que definir ownership del módulo, contrato público de inventario, entidad base de stock o existencia, estado base de disponibilidad, movimiento mínimo de inventario cuando aplique, archivos a crear o modificar dentro de src/features/inventory, y validación de permisos, tenancy, tests, documentación y trazabilidad en git. úsala para evitar mezclar inventory con procurement, production, reporting o automation y para impedir acceso caótico al dominio de inventario desde páginas o componentes.
---

# ChefOS v3 Inventory

Trabaja solo sobre tareas concretas, cerradas y revisables del módulo `inventory` de ChefOS v3.

Tu función es ayudar a definir, revisar o aterrizar cambios del módulo `inventory` dentro de la arquitectura oficial del proyecto.

No rehagas el dominio de inventario completo.
No mezcles `inventory` con otros módulos.
No abras rediseños generales.
No conviertas `inventory` en contenedor genérico de “todo lo operativo”.
...
- `.ai/sprints/sprint-06-inventory.md`
...
- procurement como dominio propio
...
5. No metas lógica de `inventory` en `src/app/`.
6. No metas lógica de `inventory` en `src/components/`.
7. No uses `src/lib/` como destino por defecto para lógica de inventario.
8. No mezcles reglas de `inventory` con `procurement`, `production`, `reporting` o `automation` sin contrato público claro.
...
### 2. Validar ownership dentro de `inventory`

Debes responder:

- si la tarea pertenece realmente a `inventory`
...
No permitas que `inventory` absorba lógica ajena por comodidad.
...
- cuál es la entidad base de stock o existencia afectada
- cuál es su estado base de disponibilidad
- si interviene movimiento mínimo o no
...
- cómo manual de Git sin mezclar otras cosas
...
- deja claro qué pertenece y qué no pertenece a `inventory`
- protege la frontera pública del módulo
- evita acceso disperso al dominio de inventario desde cualquier capa
