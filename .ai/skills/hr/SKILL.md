---
name: hr
description: ayuda a trabajar una tarea concreta del módulo hr de chefos v3. úsala cuando haya que definir ownership del módulo, contrato público de recursos humanos, entidad base de personal, asignación funcional mínima, estado base de personal, datos mínimos operativos de rrhh, archivos a crear o modificar dentro de src/features/hr, y validación de permisos, tenancy, tests, documentación y trazabilidad en git. úsala para evitar mezclar hr con identity, commercial, compliance o automation y para impedir acceso caótico al dominio de personal desde páginas o componentes.
---

# ChefOS v3 HR

Trabaja solo sobre tareas concretas, cerradas y revisables del módulo `hr` de ChefOS v3.

Tu función es ayudar a definir, revisar o aterrizar cambios del módulo `hr` dentro de la arquitectura oficial del proyecto.

No rehagas el dominio de recursos humanos completo.
No mezcles `hr` con otros módulos.
No abras rediseños generales.
No conviertas `hr` en contenedor genérico de “todo lo relacionado con usuarios”.
...
## Ownership del módulo `hr`

El módulo `hr` es owner de:
...
- reglas mínimas del dominio de personal
...
- compliance como dominio propio
...
5. No metas lógica de `hr` en `src/app/`.
6. No metas lógica de `hr` en `src/components/`.
7. No uses `src/lib/` como destino por defecto para lógica de recursos humanos.
8. No dupliques lógica fuente de otros módulos dentro de `hr` si existe contrato público.
...
### 2. Validar ownership dentro de `hr`

Debes responder:

- si la tarea pertenece realmente a `hr`
...
No permitas que `hr` absorba lógica ajena por comodidad.
...
- cuál es la entidad base de personal afectada
- cuál es su asignación funcional mínima
- cuál es su estado base
- qué datos mínimos operativos toca el cambio
...
- deja claro qué pertenece y qué no pertenece a `hr`
- protege la frontera pública del módulo
- evita acceso disperso al dominio de personal desde cualquier capa
