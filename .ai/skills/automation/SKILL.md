---
name: automation
description: ayuda a trabajar una tarea concreta del módulo automation de chefos v3. úsala cuando haya que definir ownership del módulo, contrato público de automatización, regla base de automatización, trigger mínimo, condición mínima, acción automática mínima, archivos a crear o modificar dentro de src/features/automation, y validación de permisos, tenancy, tests, documentación y trazabilidad en git. úsala para evitar mezclar automation con notifications, integrations, reporting o lógica fuente de otros módulos y para impedir acceso caótico al dominio de automatización desde páginas o componentes.
---

# ChefOS v3 Automation

Trabaja solo sobre tareas concretas, cerradas y revisables del módulo `automation` de ChefOS v3.

Tu función es ayudar a definir, revisar o aterrizar cambios del módulo `automation` dentro de la arquitectura oficial del proyecto.

No rehagas el dominio de automatización completo.
No mezcles `automation` con otros módulos.
No abras rediseños generales.
No conviertas `automation` en contenedor genérico de “cualquier proceso automático”.
...
- `.ai/sprints/sprint-10-automation.md`
...
## Ownership del módulo `automation`

El módulo `automation` es owner de:
...
- validaciones mínimas del dominio de automatización
...
5. No metas lógica de `automation` en `src/app/`.
6. No metas lógica de `automation` en `src/components/`.
7. No uses `src/lib/` como destino por defecto para lógica de automatización.
8. No dupliques lógica fuente de otros módulos dentro de `automation` si existe contrato público.
...
### 2. Validar ownership dentro de `automation`

Debes responder:

- si la tarea pertenece realmente a `automation`
...
No permitas que `automation` absorba lógica ajena por comodidad.
...
- cuál es la regla base de automatización afectada
- cuál es su trigger mínimo
- cuál es su condición mínima
- cuál es su acción automática mínima
...
- deja claro qué pertenece y qué no pertenece a `automation`
- protege la frontera pública del módulo
- evita acceso disperso al dominio de automatización desde cualquier capa
