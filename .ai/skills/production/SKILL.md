---
name: production
description: ayuda a trabajar una tarea concreta del módulo production de chefos v3. úsala cuando haya que definir ownership del módulo, contrato público de producción, entidad base de producción, estado base de ejecución productiva, transformación operativa mínima cuando aplique, archivos a crear o modificar dentro de src/features/production, y validación de permisos, tenancy, tests, documentación y trazabilidad en git. úsala para evitar mezclar production con recipes, inventory, procurement o automation y para impedir acceso caótico al dominio de producción desde páginas o componentes.
---

# ChefOS v3 Production

Trabaja solo sobre tareas concretas, cerradas y revisables del módulo `production` de ChefOS v3.

Tu función es ayudar a definir, revisar o aterrizar cambios del módulo `production` dentro de la arquitectura oficial del proyecto.

No rehagas el dominio de producción completo.
No mezcles `production` con otros módulos.
No abras rediseños generales.
No conviertas `production` en contenedor genérico de “todo lo operativo”.
...
## Ownership del módulo `production`

El módulo `production` es owner de:
...
- datos mínimos del dominio de producción
- validaciones mínimas del proceso productivo
...
5. No metas lógica de `production` en `src/app/`.
6. No metas lógica de `production` en `src/components/`.
7. No uses `src/lib/` como destino por defecto para lógica de producción.
8. No mezcles reglas de `production` con `recipes`, `inventory`, `procurement` o `automation` sin contrato público claro.
...
### 2. Validar ownership dentro de `production`

Debes responder:

- si la tarea pertenece realmente a `production`
...
No permitas que `production` absorba lógica ajena por comodidad.
...
- cuál es la entidad base de producción afectada
- cuál es su estado base de ejecución
- si interviene transformación operativa mínima o no
...
- deja claro qué pertenece y qué no pertenece a `production`
- protege la frontera pública del módulo
- evita acceso disperso al dominio de producción desde cualquier capa
