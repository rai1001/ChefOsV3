---
name: compliance
description: ayuda a trabajar una tarea concreta del módulo compliance de chefos v3. úsala cuando haya que definir ownership del módulo, contrato público de cumplimiento, control base de compliance, estado base de cumplimiento, validación obligatoria mínima, archivos a crear o modificar dentro de src/features/compliance, y validación de permisos, tenancy, tests, documentación y trazabilidad en git. úsala para evitar mezclar compliance con identity, reporting, automation o integrations y para impedir acceso caótico al dominio de cumplimiento desde páginas o componentes.
---

# ChefOS v3 Compliance

Trabaja solo sobre tareas concretas, cerradas y revisables del módulo `compliance` de ChefOS v3.

Tu función es ayudar a definir, revisar o aterrizar cambios del módulo `compliance` dentro de la arquitectura oficial del proyecto.

No rehagas el dominio de compliance completo.
No mezcles `compliance` con otros módulos.
No abras red generales.
No conviertas `compliance` en contenedor genérico de “cualquier validación”.
...
- `.ai/sprints/sprint-09-compliance.md`
...
## Ownership del módulo `compliance`

El módulo `compliance` es owner de:
...
5. No metas lógica de `compliance` en `src/app/`.
6. No metas lógica de `compliance` en `src/components/`.
7. No uses `src/lib/` como destino por defecto para lógica de compliance.
8. No dupliques lógica fuente de otros módulos dentro de `compliance` si existe contrato público.
...
### 2. Validar ownership dentro de `compliance`

Debes responder:

- si la tarea pertenece realmente a `compliance`
...
No permitas que `compliance` absorba lógica ajena por comodidad.
...
- cuál es el control base de compliance afectado
- cuál es su estado base de cumplimiento
- cuál es la validación obligatoria mínima
- si interviene trazabilidad mínima o no
...
- deja claro qué pertenece y qué no pertenece a `compliance`
- protege la frontera pública del módulo
- evita acceso disperso al dominio de cumplimiento desde cualquier capa
