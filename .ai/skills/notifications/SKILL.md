---
name: notifications
description: ayuda a trabajar una tarea concreta del módulo notifications de chefos v3. úsala cuando haya que definir ownership del módulo, contrato público de notificaciones, notificación base, estructura mínima de mensaje, canal mínimo de entrega, estado base de notificación, archivos a crear o modificar dentro de src/features/notifications, y validación de permisos, tenancy, tests, documentación y trazabilidad en git. úsala para evitar mezclar notifications con automation, integrations, reporting o lógica fuente de otros módulos y para impedir acceso caótico al dominio de notificaciones desde páginas o componentes.
---

# ChefOS v3 Notifications

Trabaja solo sobre tareas concretas, cerradas y revisables del módulo `notifications` de ChefOS v3.

Tu función es ayudar a definir, revisar o aterrizar cambios del módulo `notifications` dentro de la arquitectura oficial del proyecto.

No rehagas el dominio de notificaciones completo.
No mezcles `notifications` con otros módulos.
No abras rediseños generales.
No conviertas `notifications` en contenedor genérico de “cualquier comunicación del sistema”.
...
- `.ai/sprints/sprint-11-notifications.md`
...
## Ownership del módulo `notifications`

El módulo `notifications` es owner de:
...
- validaciones mínimas del dominio de notificaciones
...
5. No metas lógica de `notifications` en `src/app/`.
6. No metas lógica de `notifications` en `src/components/`.
7. No uses `src/lib/` como destino por defecto para lógica de notificaciones.
8. No dupliques lógica fuente de otros módulos dentro de `notifications` si existe contrato público.
...
### 2. Validar ownership dentro de `notifications`

Debes responder:

- si la tarea pertenece realmente a `notifications`
...
No permitas que `notifications` absorba lógica ajena por comodidad.
...
- cuál es la notificación base afectada
- cuál es su estructura mínima de mensaje
- cuál es su canal mínimo de entrega
...
- deja claro qué pertenece y qué no pertenece a `notifications`
- protege la frontera pública del módulo
- evita acceso disperso al dominio de notificaciones desde cualquier capa
