---
name: catalog
description: ayuda a trabajar una tarea concreta del módulo catalog de chefos v3. úsala cuando haya que definir ownership del módulo, contrato público de catálogo, entidad catalogable base, estructura mínima de clasificación o visibilidad, estado base de catálogo, archivos a crear o modificar dentro de src/features/catalog, y validación de permisos, tenancy, tests, documentación y trazabilidad en git. úsala para evitar mezclar catalog con commercial, recipes, inventory o integrations y para impedir acceso caótico al dominio de catálogo desde páginas o componentes.
---

# ChefOS v3 Catalog

Trabaja solo sobre tareas concretas, cerradas y revisables del módulo `catalog` de ChefOS v3.

Tu función es ayudar a definir, revisar o aterrizar cambios del módulo `catalog` dentro de la arquitectura oficial del proyecto.

No rehagas el dominio de catálogo completo.
No mezcles `catalog` con otros módulos.
No abras rediseños generales.
No conviertas `catalog` en contenedor genérico de “todo lo publicable”.
...
- `.ai/sprints/sprint-04-catalog.md`
...
## Ownership del módulo `catalog`

El módulo `catalog` es owner de:

- entidad catalogable base
- estructura mínima de clasificación o visibilidad
- estado base de catálogo
- reglas mínimas del dominio de catálogo
- validaciones mínimas sobre publicación o pertenencia al catálogo
...
- commercial como dominio propio
...
5. No metas lógica de `catalog` en `src/app/`.
6. No metas lógica de `catalog` en `src/components/`.
7. No uses `src/lib/` como destino por defecto para lógica de catálogo.
8. No mezcles reglas de `catalog` con `commercial`, `recipes`, `inventory` o `integrations` sin contrato público claro.
...
### 2. Validar ownership dentro de `catalog`

Debes responder:

- si la tarea pertenece realmente a `catalog`
...
No permitas que `catalog` absorba lógica ajena por comodidad.
...
- cuál es la entidad catalogable base afectada
- cuál es su estructura mínima de clasificación o visibilidad
...
- deja claro qué pertenece y qué no pertenece a `catalog`
- protege la frontera pública del módulo
- evita acceso disperso al dominio de catálogo desde cualquier capa
