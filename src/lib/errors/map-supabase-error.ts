...
export function mapSupabaseError(
  ...
  const rawMessage = e?.message ?? (raw instanceof Error ? raw.message : undefined) ?? 'Error desconocido'
  ...
  switch (code) {
    case '23505': // unique_violation
      return new ConflictError(undefined, { cause: raw })
    case '23503': // foreign_key_violation
      return new ConflictError(undefined, { cause: raw })
    case '23502': // not_null_violation
    case '23514': // check_violation
      return new ValidationError(undefined, undefined, { cause: raw })
    case '42501': // insufficient_privilege (RLS deniega)
      return new ForbiddenError(undefined, { cause: raw })
    case 'PGRST116': // PostgREST: no rows where one expected
      return new NotFoundError(context?.resource ?? 'recurso', undefined, { cause: raw })
    case 'P0003': // ChefOS custom: no active hotel
      return new ForbiddenError(undefined, { cause: raw })
  }

  // Supabase Auth status codes
  if (status === 401) return new UnauthorizedError(undefined, { cause: raw })
  if (status === 403) return new ForbiddenError(undefined, { cause: raw })
  if (status === 404) return new NotFoundError(context?.resource ?? 'recurso', undefined, { cause: raw })
  if (status === 409) return new ConflictError(undefined, { cause: raw })
  if (status === 422) return new ValidationError(undefined, undefined, { cause: raw })
  ...
  if (lower.includes('unauthorized') || lower.includes('no session')) {
    return new UnauthorizedError(undefined, { cause: raw })
  }

  return new InfrastructureError(undefined, { cause: raw })
}
