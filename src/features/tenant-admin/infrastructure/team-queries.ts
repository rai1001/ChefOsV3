 ...
 * Usa RPC `v3_get_team_members` si existe en Supabase; fallback a SELECT join
 * sobre v3_memberships + profiles si no (email from auth.users no es accesible vía RLS).
 */
export async function fetchTeamMembers(
  ...
  const rpcResp = await supabase.rpc('v3_get_team_members', { p_hotel_id: hotelId })
  ...
  // se cree la RPC v3_get_team_members en una migración adicional (fuera de sprint-02b).
  const { data, error } = await supabase
    .from('v3_memberships')
    .select('id, user_id, role, is_active, is_default, created_at, profiles:v3_profiles(full_name)')
    .eq('hotel_id', hotelId)
