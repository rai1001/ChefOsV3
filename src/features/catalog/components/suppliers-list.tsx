...
export function SuppliersList({ hotelId }: { hotelId: string }) {
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto">
          <Button asChild size="sm">
            <Link href="/catalog/suppliers/new">Nuevo proveedor</Link>
          </Button>
        </div>
      </div>

      {isLoading ? (
        <p className="kpi-label">Cargando proveedores…</p>
      ) : rows.length === 0 ? (
        <p className="text-[color:var(--color-text-muted)]">No hay proveedores.</p>
      ) : (
        <div
          ...
          <table className="w-full text-sm">
            <thead>
              ...
              >
                ...
                <th className="kpi-label px-3 py-2 text-right">Rating</th>
                  ...
                  <td className="px-3 py-2 text-right">
                    {s.is_active ? (
                      <Button
                        size="sm"
                        variant="secondary"
                        onClick={() => archive.mutate({ hotelId, supplierId: s.id })}
                      ...
                      <Button
                        size="sm"
                        variant="secondary"
                        onClick={() => restore.mutate({ hotelId, supplierId: s.id })}
